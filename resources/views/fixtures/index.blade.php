...
<form method="get" class="filters-bar">
    ...
    <select name="competition" class="filter-select">
        ...
        @foreach(array_unique(array_map(fn($m)=>$m['competition']['name']??'', $fixtures['fixtures']??[])) as $comp)
            @if($comp)
    ...
    <select name="categorie" class="filter-select">
        ...
        <option value="En cours" @if(request('categorie')=='En cours')selected @endif>En cours</option>
        <option value="Terminé" @if(request('categorie')=='Terminé')selected @endif>Terminé</option>
        <option value="À venir" @if(request('categorie')=='À venir')selected @endif>À venir</option>
        <option value="Reporté" @if(request('categorie')=='Reporté')selected @endif>Reporté</option>
        <option value="Annulé" @if(request('categorie')=='Annulé')selected @endif>Annulé</option>
        <option value="Suspendu" @if(request('categorie')=='Suspendu')selected @endif>Suspendu</option>
    </select>
...
if(request('categorie')) {
    $filtered = array_filter($filtered, function($m) use ($statusTrad) {
        $status = $statusTrad[$m['status']] ?? $m['status'];
        return $status == request('categorie');
    });
}
...
foreach($filtered as $fixture) {
    $status = $statusTrad[$fixture['status']] ?? $fixture['status'];
    $grouped[$status][] = $fixture;
}

/* --- Forcer l'ordre des sections --- */
$order = ['En cours', 'Terminé', 'À venir', 'Reporté', 'Annulé', 'Suspendu'];
...
@forelse($grouped as $statusLabel => $games)
    <div class="matches-section">
        <h2 class="section-title">{{ $statusLabel }}</h2>
                    ...
                    <div class="teams-row" style="display:flex;justify-content:space-between;align-items:center;width:100%;margin-bottom:12px;gap:10px;">
                        <span class="team-name" style="display:flex;align-items:center;gap:6px;max-width:48%;overflow:hidden;text-overflow:ellipsis;">
                            @if(isset($fixture['homeTeam']['crest']))
                                <img src="{{ $fixture['homeTeam']['crest'] }}" alt="logo" style="width:28px;height:28px;border-radius:50%;background:#f3f4f6;box-shadow:0 2px 8px rgba(0,0,0,0.07);">
                            @endif
                            <span style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $fixture['homeTeam']['name'] }}</span>
                        </span>
                        <span class="score" style="min-width:56px;">
                        ...
                        <span class="team-name" style="display:flex;align-items:center;gap:6px;max-width:48%;overflow:hidden;text-overflow:ellipsis;justify-content:flex-end;">
                            @if(isset($fixture['awayTeam']['crest']))
                                <img src="{{ $fixture['awayTeam']['crest'] }}" alt="logo" style="width:28px;height:28px;border-radius:50%;background:#f3f4f6;box-shadow:0 2px 8px rgba(0,0,0,0.07);">
                            @endif
                            <span style="white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $fixture['awayTeam']['name'] }}</span>
                        </span>
                    </div>

                    <div class="match-date">
                        {{ Carbon::parse($fixture['utcDate'])->locale('fr')->translatedFormat('d F Y, H:i') }}
                    </div>
